...
Read-модель — **отдельное хранилище**, оптимизированное под запросы на чтение.
Заполняется через проекции, подписанные на доменные события.
...
```
Aggregate ──▶ BusAdapter (IObserver) ──▶ IEventBus.Publish() ──▶ Потребители
```
...
```jsonc
{
  // === Строка подключения к Command DB (Event Store) ===
  "ConnectionStrings": {
...
| Пакет | Версия |
|-------|--------|
| `DigiTFactory.Libraries.SeedWorks` | 0.3.0 |
...
7. **Storage** — настроить DbContext, Repository, маппинг событий
8. **Api** — сконфигурировать DI (Autofac + MediatR + EventStore + EventBus), создать контроллеры
9. **Тесты** — написать unit-тесты на бизнес-логику агрегатов
10. **CI/CD** — настроить GitHub Actions для сборки и деплоя
